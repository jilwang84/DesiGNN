...
class LLMPromptTemplate:
    @staticmethod
    def generate_task_similarity_prompt_parser(unseen_dataset_description, benchmark_dataset_descriptions,
        ...
        prompt = ChatPromptTemplate.from_messages(
            [("system", "You are a machine learning expert proficient in graph neural network design and graph dataset "
                        ...
                        "the optimal GNN model architecture. Your ultimate goal is to recommend a GNN model "
                        "architecture that performs well on the unseen dataset based on the best GNN model "
                        "architectures on similar benchmark datasets to the user.\nFor your information, a classical "
                        "task similarity metric in graph neural networks is measured by how different GNN "
                        "architectures perform across various datasets. A set of diverse GNN designs, known as anchor "
                        ...
                        "Kendall rank correlation. This metric helps transfer the best GNN architectures or "
                        "principles across similar datasets."),
             ("user", "{input}")]
        )
        user_input = "Based on the given descriptions of the unseen dataset and multiple benchmark datasets below, " \
                     "please take a deep breath and work on this problem step-by-step: analyze the characteristics " \
                     "of datasets that affect their choice of best GNN architectures in practice and estimate the " \
                     "task similarity scores between the unseen dataset and each benchmark dataset. The task " \
                     "similarity score should range from -1 (completely dissimilar tasks) to 1 (identical tasks). " \
        ...
        similarity_tool.__doc__ = "Estimate and justify the task similarity scores between the unseen dataset and " \
                                  "each benchmark dataset based on the user-entered dataset descriptions and " \
                                  "statistics."
    ...
    @staticmethod
    def generate_task_similarity_prompt(unseen_dataset_description, benchmark_dataset_descriptions,
                                            similarity_scores=None):
        """
        Generates a prompt that combines textual and mathematical similarities.
        :param unseen_dataset_description: Description of the unseen dataset.
        ...
        prompt += "Based on the summary above and the given descriptions of various graph datasets below, evaluate " \
                  "and provide the task similarity scores between the unseen dataset and each of the benchmark " \
                  "datasets. Consider the characteristics such as graph size, density, types of node and edge " \
                  "attributes, and the nature of prediction tasks (e.g., node classification, link prediction) " \
                  "described in each dataset summary or in your own knowledge base.\n\n"
    ...
    @staticmethod
    def generate_task_similarity_prompt_llama(unseen_dataset_description, benchmark_dataset_descriptions, dataset_name):
        # # Summary of the task similarity metric concept
        # metric_summary = "Task similarity in graph neural networks is measured based on how different GNN designs " \
        #                  "perform across various tasks. A set of diverse GNN designs, known as anchor models, are " \
        #                  "applied to these tasks to capture different aspects. By comparing the performance rankings " \
        ...
        # Introduction to the LLM task
        prompt = "Task similarity in graph neural networks is measured based on how different GNN designs perform across various tasks. A set of diverse GNN designs, known as anchor models, are applied to these tasks to capture different aspects. By comparing the performance rankings of these models across tasks using the Kendall rank correlation, we can quantify task similarity. This metric helps in transferring the best GNN designs or principles across similar tasks.\nBased on the summary above and the given topological descriptions of various graph datasets below, please provide the task similarity scores between the unseen dataset and the benchmark datasets. Please pay special attention to the similarities between the graph topology listed in each dataset description, which are empirically found to be consistent with good GNN models across different datasets.\n"

        prompt += "Unseen Dataset's Description:\n"
        unseen_dataset_description = unseen_dataset_description.replace("\n\n", "\n")
        prompt += f"{unseen_dataset_description}\n"

        prompt += "Benchmark Datasets' Descriptions:\n"
        description = benchmark_dataset_descriptions.replace("\n\n", "\n")
        prompt += f"{description}"

        # Instruct the LLM on the expected output format
        prompt += "Please provide a task similarity score ranging from -1 (completely dissimilar tasks) to 1 (identical tasks) to measure the topological similarity between the unseen dataset and the benchmark dataset with respect to tailored GNN design patterns. Please first report the task similarity score and then provide a brief justifications.\n"
                #   "for each score based on the dataset summary and the task similarity metric concept. For example:\n"
        # prompt += f"Similarity scores:\n- Unseen Dataset and {dataset_name}: XXX\n"
        # prompt += f"\nJustification:\n...\nNow, please provide the task similarity scores between the unseen dataset and each of the benchmark datasets:"
